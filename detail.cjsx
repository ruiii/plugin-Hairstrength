...
Detail = React.createClass
  render: ->
    ...
    time = rate = ranking = rateDelta =  [0, 0]
    ...
    if accounted
      ...
      rate = baseDetail.presumedSenka
      rate1 = data[data.length - 1][2]
    else
      text = __ 'Rate'
      rate = data[data.length - 1][2]
      if data[data.length - 2]?
        rate1 = data[data.length - 2][2]
      else
        rate = ''
